...
def is_consonant(syllable):
	return syllable in consonant_list


def load_our_pinyin_dict():
	load_phrases_dict({'还清': [['huán'], ['qīng']]})
	load_phrases_dict({'还未': [['hái'], ['wèi']]})
	load_phrases_dict({'先还': [['xiān'], ['huán']]})
	load_phrases_dict({'银行': [['yín'], ['háng']]})
	load_phrases_dict({'行人': [['xíng'], ['rén']]})
	load_phrases_dict({'行走': [['xíng'], ['zǒu']]})
	load_phrases_dict({'还款额': [['huán'], ['kuǎn'], ['é']]})


def pinyin_format(syllable):
...
def txt2pinyin(txt):
	...
	for item in pinyin_list:
		...
		phone_list.append(separate_syllable(pinyin_normalized))
	return phone_list, pinyin_sequence
