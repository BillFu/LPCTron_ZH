...
int main()
{
    ...
    if (AudioStreamIndex >= 0)
    {
        ...
        AVStream* stream = ifmt_ctx->streams[AudioStreamIndex];
    ...
    while (1)
    {
        ...
        if (AudioStreamIndex == pkt_in.stream_index)
        {
            ...
            ret = avcodec_decode_audio4(pCodecCtx, frame, &got_frame, &pkt_in);
            if (ret < 0)
            {
                av_frame_free(&frame);
            ...
            if (got_frame)
            {
                ...
                if (isSame)
                {
                    ...
                    swr_convert(audio_convert_ctx, &buffer, MAX_AUDIO_FRAME_SIZE, (const uint8_t **) frame->data,
                                frame->nb_samples < 1000 ? 1152 : frame->nb_samples);
