...
Status ReadTensorFromImageFile(const string& file_name,
        ...
        vector<Tensor>* out_tensors, bool writeGraph)
{
    ...
    auto image_reader = DecodeJpeg(root.WithOpName("jpeg_reader"),
            file_reader, DecodeJpeg::Channels(wanted_channels));
    ...
    auto resized = ResizeBilinear(root.WithOpName("size"), dims_expander,
            Const(root, {input_height, input_width}));
