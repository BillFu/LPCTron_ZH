...
"""
py_line: neng2 bei4 xuan3 zuo4 ji1 xin1 ge2 zi1 liao4 de5 
	shou1 cang2 zhe3 ye1 lu3 shen1 gan3 rong2 xing4
	
ipa_line: nəŋ2 pei4 ɕyɛn3 tsuo4 tɕi1 ɕin1 kɤ2 tsɿ1 liɑu4 tɤ5 
	ʂou1 tsʰɑŋ2 tʂɤ3 iɛ1 lu3 ʂən1 kan3 ruŋ2 ɕiŋ4
"""
######################## 全局变量 ########################
# 三种字符集标志
...
def proc_one_sentence(hanzi_line, py_line):
	punctuations_info = parse_hanzi_line(hanzi_line)
	ipa_line = py2ipa(py_line)
	...
	cur_punct_info = remained_punctuations_info.pop(0) # from the beginning
	cur_punct_char = cur_punct_info[0]
	cur_punct_index = cur_punct_info[1]

	new_ipa_list = []
	for index, ipa in enumerate(ipa_list):
		new_ipa_list.append(ipa)

		if index == cur_punct_index:
			new_ipa_list.append(cur_punct_char)
			if len(remained_punctuations_info) == 0:
				cur_punct_char = '。'
				cur_punct_index = len(ipa_list) - 1
			else:
				cur_punct_info = remained_punctuations_info.pop(0)  # from the beginning
				cur_punct_char = cur_punct_info[0]
				cur_punct_index = cur_punct_info[1]
...
def test():
	...
	py_line = "neng2 bei4 xuan3 zuo4 ji1 xin1 ge2 zi1 liao4 de5 shou1 cang2 zhe3 ye1 lu3 shen1 gan3 rong2 xing4"
	# "zuo2 ri4 zhe4 ming2 shang1 zhe2 yu3 yi1 sheng1 quan2 bu4 bei4 jing3 fang1 yi1 fa3 xing2 shi4 ju1 liu2"
	ipa_line = convertLine(py_line)
	print("py_line: {}".format(py_line))
	print("ipa_line: {}".format(ipa_line))
"""
...
def main():
	...
	out_file = open(args.out_ipa_file, 'w')

	with open(args.in_pinyin_file, 'r') as f:
		while True:
			line1 = f.readline()
			if not line1:
				break
			...
			ipa_line = proc_one_sentence(hanzi_line, py_line)
