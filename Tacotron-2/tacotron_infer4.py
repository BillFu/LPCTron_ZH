...
def inference(sentence_id, normalized_hz_line, synthesizer,
			mutex_semaphore, mmap_file, JOBDES_SECTION_SIZE):
	...
	frame_num = mels.shape[0]  # the total number of frames generated.
	...
	job_des_str = json.dumps(job_descriptor)
	job_des_bytes = job_des_str.encode()
	mmap_file.write(job_des_bytes)

	mmap_file.seek(JOBDES_SECTION_SIZE)
...
def main():
	...
	# TASKINFO_SHMEM_NAME = "/Taskinfo_SHM1"
	JOBDES_SECTION_SIZE = 1024
	FEATURE_SHMEM_SIZE = JOBDES_SECTION_SIZE + 2000 * 20 * 4   # 2000 is max number of frames
